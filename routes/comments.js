...
router.post('/posts/:postId/comments', authMiddleware, async(req, res) => {
    try {
        ...
        const user = res.locals.user; // 토큰을 검사하여, 유효한 토큰일 경우에만 댓글 작성 가능
        // 게시글 미존재
        if (!postId) {
            res.status(404).json({
...
router.put("/posts/:postId/comments/:commentId", authMiddleware, async(req, res) => {
    try {
        ...
        const user = res.locals.user; // 토큰을 검사하여, 유효한 토큰일 경우에만 댓글 수정 가능
        // console.log(user);        
